...
## 优化表选择的机制
现在都是list_table，然后一把传给大模型。可以选择智能一点的agentic_search，也就是像claude code，一点点找代码一样，根据需要查的内容一点点的搜寻表。也可以参考前面的记忆
实在找不准了再把所有表都给大模型。

## 写sql时有语法提示和自动联想
写sql时，会根据数据库中的表信息，进行自动联想

## SQL编辑器增加单条执行的功能
方便存入多条sql，然后自然语言转化成SQL以后追加到最后一条

## ai agent交互优化
ai agent打开的位置到右上角，且默认打开。
支持多个会话。且会话会存储到sqlite数据库中
